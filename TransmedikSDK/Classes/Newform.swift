...
protocol newformsViewControllerprotocol {
    func refresh()
}


class newformsViewController: UIViewController {
    ...
    @IBOutlet weak var highttable: NSLayoutConstraint!
    ...
    var delegaterefresh : newformsViewControllerprotocol?
    var klinik = ""
    var spesialis = ""
    ...
    override func viewDidLoad() {
        ...
        self.highttable.constant = tables.contentSize.height
...
extension newformsViewController : UITableViewDataSource ,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            case "text","number","email","decimal":
                ...
                }else if list[indexPath.row].input_type == "decimal" {
                    cell.nametext.keyboardType = .decimalPad
                }
...
//                    keluar(view: presentPage)
                    ...
                    //
//                    self.db.request { (user) in
//                        if user != nil {
//                            let _api = AssesmentObj()
//                            _api.postassesment(user![0].token!, answer: stringform) { (data) in
//                                self.delegates?.kirim(list: nil)
