    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        dismiss(animated: true, completion: {
            if self.mediaType == "video" {
                ...
                if let pickedImage = info[self.convertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.editedImage)] as? UIImage {
                    if let image1 = pickedImage.resizeWithWidth(width: AppSettings.maxImageWidth) {
                        self.sendimage(dataimage: image1)

                    }
                }
                else if let pickedImage = info[self.convertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.originalImage)] as? UIImage {
                    if let image1 = pickedImage.resizeWithWidth(width: AppSettings.maxImageWidth) {
                        self.sendimage(dataimage: image1)
